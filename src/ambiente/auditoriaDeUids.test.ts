...
describe('código real de cada crachá, na pasta', () => {
  it('uma linha por crachá, na primeira leitura, com o hash que liga ao vínculo', async () => {
    ...
    expect(await anotarUid(handle, SAL, UID, new Date('2026-09-24T13:00:00Z'), 'Dongle USB')).toBe(true)
    expect(await anotarUid(handle, SAL, UID, new Date('2026-09-24T13:05:00Z'), 'Dongle USB')).toBe(false)
  ...
  it('não repete um crachá que já estava no arquivo de outra abertura do app', async () => {
    const { handle } = criarPastaFalsa()
    await anotarUid(handle, SAL, UID, new Date(), 'Dongle USB')
    esquecerAuditoria()
    expect(await anotarUid(handle, SAL, UID, new Date(), 'Dongle USB')).toBe(false)
  })

  it('dois crachás quase juntos, o mesmo crachá duas vezes: uma linha só', async () => {
    const { handle } = criarPastaFalsa()
    const [a, b] = await Promise.all([
      anotarUid(handle, SAL, UID, new Date(), 'Dongle USB'),
      anotarUid(handle, SAL, UID, new Date(), 'Dongle USB'),
    ])
