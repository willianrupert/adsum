// Bancada: o PN532 em pinos livres, fora da UART0.
//
// O `diag_uart4` usou GPIO 20 e 21, que no ESP32-C3 são a UART0 — a porta do
// log de boot. Isso estraga as duas pontas do diagnóstico: a ROM mantém o
// GPIO 21 em nível alto por conta própria (então "repouso alto" não prova
// que o módulo está lá), e o log de boot é despejado dentro do RX do PN532.
//
// Aqui o módulo vai para GPIO 4 (RX) e GPIO 5 (TX), que são livres no
// SuperMini e não são pinos de strapping (2, 8 e 9 são).
//
// Por que o PN532 não responde.
//
...
void setup() {
  Serial.begin(115200);
  delay(1500);
  Serial.println("#DIAG diag_uart5 — PN532 em pinos livres (4 e 5)");
  medirRepouso();
  configurar();
}
